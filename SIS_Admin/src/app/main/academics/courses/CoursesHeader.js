...
function CoursesHeader({ showSearchAddButton = true, isHomePage = true }) {
  ...
  const _handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      dispatch(getCourses({ search: e.target.value }));
    }
  }
  return (
    <div className="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-1 w-full items-center justify-between py-32 px-24 md:px-32">
      <Typography
        ...
        to={`${!isHomePage && "/academics/courses"}`}
      >
        {!isHomePage && (
          <FuseSvgIcon size={20}>
            {theme.direction === "ltr"
      ...
      {showSearchAddButton && (
        <div className="flex flex-col w-full sm:w-auto sm:flex-row space-y-16 sm:space-y-0 flex-1 items-center justify-end space-x-8">
          ...
          {!user?.role?.includes(authRoles.instructors?.[0]) && (
            <motion.div
