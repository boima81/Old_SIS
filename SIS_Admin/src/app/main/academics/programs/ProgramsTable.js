...
import StudentsTableHead from "./ProgramsTableHead";
import {
  deleteProgram,
  getPrograms,
  selectPrograms,
...
function ProgramsTable(props) {
  ...
  useEffect(() => {
    setLoading(true);
    dispatch(getPrograms({})).then(() => setLoading(false));
  }, [dispatch]);
  ...
  function handleDeselect() {
    dispatch(deleteProgram(selected));
    setSelected([]);
    setOpenModal(!openModal);
  }

  function handleClick(item) {
    props.navigate(`/academics/programs/${item.id}`);
  }
...
export default withRouter(ProgramsTable);
