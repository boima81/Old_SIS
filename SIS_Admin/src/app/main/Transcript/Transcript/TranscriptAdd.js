...
function TranscriptAdd(props) {
  ...
  const schema = yup.object().shape({
    feesName: yup.string().required("Transcript name is required"),
    feesAmount: yup.string().required("Transcript amount is required"),
  });
  const {
    control,
    handleSubmit,
          ...
          <div className="space-y-32">
            <Controller
              ...
              render={({ field }) => (
                <MainTextField
                  data={field}
                  label="Transcript amount"
                  className="md:w-1/2"
                  type="number"
                  placeholder="Transcript amount"
...
export default withRouter(TranscriptAdd);
