...
function ProductsHeader({ showSearchAddButton = true, isHomePage = true }) {
  ...
  const _handleKeyDown = (e) => {
    if (e.key === 'Enter') {
      dispatch(getApplications({ search: e.target.value }))
    }
  }
  return (
    <div className="flex flex-col sm:flex-row space-y-16 sm:space-y-0 flex-1 w-full items-center justify-between py-32 px-24 md:px-32">
      <Typography
        ...
        to={`${!isHomePage && "/admission/application"}`}
      >
        {!isHomePage && (
          <FuseSvgIcon size={20}>
            {theme.direction === "ltr"
